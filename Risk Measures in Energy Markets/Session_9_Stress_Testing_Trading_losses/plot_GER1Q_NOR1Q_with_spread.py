...
def calculate_statistics(df: pd.DataFrame) -> pd.DataFrame:
    stats = df[["GER1Q", "NOR1Q", "Difference_GER1Q_NOR1Q"]].describe().T
...
def create_chart(input_path: Path, sheet_name: str, output_path: Path) -> pd.DataFrame:
    ...
    df["Difference_GER1Q_NOR1Q"] = df["GER1Q"] - df["NOR1Q"]
    ...
    ax_main.plot(df["Date"], df["GER1Q"], label="GER1Q", linewidth=1.8, color="#1f77b4")
    ...
    ax_spread.plot(
        df["Date"],
        df["Difference_GER1Q_NOR1Q"],
        label="difference GER1Q and NOR1Q",
        linewidth=1.8,
        color="#2ca02c",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a two-panel chart for GER1Q and NOR1Q plus their difference."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    output_path = args.output or args.input.with_name("GER1Q_NOR1Q_timeseries_spread.png")
    stats_output = args.stats_output or args.input.with_name("GER1Q_NOR1Q_statistics.csv")
