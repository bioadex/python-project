...
def calculate_statistics(df: pd.DataFrame) -> pd.DataFrame:
    stats = df[["GER1M", "GER1Q", "Difference_GER1M_GER1Q"]].describe().T
...
def create_chart(input_path: Path, sheet_name: str, output_path: Path) -> pd.DataFrame:
    ...
    required_columns = {"Date", "GER1M", "GER1Q"}
    ...
    df["Difference_GER1M_GER1Q"] = df["GER1M"] - df["GER1Q"]
    ...
    ax_main.plot(df["Date"], df["GER1Q"], label="GER1Q", linewidth=1.8, color="#ff7f0e")
    ax_main.set_title("GER1M and GER1Q", fontsize=14, pad=10)
    ...
    ax_spread.plot(
        df["Date"],
        df["Difference_GER1M_GER1Q"],
        label="difference GER1M and GER1Q",
        linewidth=1.8,
        color="#2ca02c",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a two-panel chart for GER1M and GER1Q plus their difference."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    output_path = args.output or args.input.with_name("GER1M_GER1Q_timeseries_spread.png")
    stats_output = args.stats_output or args.input.with_name("GER1M_GER1Q_statistics.csv")
